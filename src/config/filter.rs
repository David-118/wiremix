...
impl Filter {
    ...
    pub fn favorite_default() -> Vec<Filter> {
        vec![]
    }
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let mut merged = if is_favorites {
            vec![]
        } else {
            Self::defaults()
        };
    ...
    pub fn merge_filters<'de, D>(
        deserializer: D,
    ) -> Result<Vec<Filter>, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        Self::merge(deserializer, false)
    }

    pub fn merge_favorites<'de, D>(
        deserializer: D,
    ) -> Result<Vec<Filter>, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        Self::merge(deserializer, true)
    }
}
